...
export async function POST(req) {
  ...
  if (!email || !email.includes("@")) return Response.json({ error: "Email tidak valid" }, { status: 400 });
  try {
    if (adminDb) {
      await adminDb.collection("subscribers").doc(email.toLowerCase()).set({
        email: email.toLowerCase(), createdAt: new Date().toISOString(), source: "website-footer",
      }, { merge: true });
    }
