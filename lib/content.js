...
export async function getConsultationDurations() {
  try {
    ...
    const normalized = data.durations
      ...
      .map((p, i) => {
        // Kompatibilitas: dokumen lama menyimpan `minutes` (angka). Kalau `duration`
        // (teks bebas) belum ada, bentuk dari angka lama supaya tetap tampil wajar.
        const duration = (p.duration || "").trim() || (p.minutes ? `${p.minutes} menit` : "");
        const name = (p.name || "").trim() || (duration ? `Konsultasi ${duration}` : `Paket ${i + 1}`);
        return {
